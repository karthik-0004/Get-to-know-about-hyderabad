"""
scrape_nobroker.py
Selenium scraper for nobroker.in Hyderabad real-estate listings.

Usage:
    python scrape_nobroker.py

Features:
  - Attempts all 126 localities with name-variant fallback.
  - Uses NoBroker's API-style search URLs.
...
OUTPUT_FILE = "nobroker_hyderabad.csv"
...
# ──────────────────────────────────────────────
# NoBroker-specific scraping
# ──────────────────────────────────────────────

def build_url(variant: str, page: int) -> str:
    """Build NoBroker buy URL for a locality in Hyderabad."""
    slug = variant.lower().replace(" ", "+")
    # NoBroker uses page offset (0-indexed pages, ~25 results per page)
    page_num = page - 1
    return (
        f"https://www.nobroker.in/property/sale/hyderabad/{variant.replace(' ', '+')}?"
        f"searchParam=city%3Dhyderabad%26locality%3D{slug}"
        f"&orderBy=nbRank&pageNo={page_num}&searchType=locality"
    )


def build_url_alt(variant: str, page: int) -> str:
    """Alternative NoBroker URL format."""
    slug = variant.lower().replace(" ", "-").replace(".", "")
    page_num = page - 1
    return f"https://www.nobroker.in/property/sale/hyderabad/{slug}?pageNo={page_num}&searchType=locality"
...
def extract_listing(card, standard_locality: str) -> Optional[Dict]:
    """Extract one listing from a NoBroker property card."""
    try:
        # Project name / title
        project_name = (
            safe_text(card, "h2")
            or safe_text(card, "[class*='heading']")
            or safe_text(card, "[class*='title']")
            or safe_text(card, "a[class*='card'] h2, a[class*='card'] h3")
            or safe_text(card, ".nb-card__header")
        )

        # Price
        price_text = (
            safe_text(card, "[class*='price']")
            or safe_text(card, "[class*='cost']")
            or safe_text(card, "[class*='nb-card__price']")
        )
        price_inr = parse_price_inr(price_text)

        # Area — NoBroker shows carpet area / super area
        area_text = (
            safe_text(card, "[class*='area']")
            or safe_text(card, "[class*='carpet']")
            or safe_text(card, "[class*='super-area']")
            or safe_text(card, "[class*='builtUp']")
        )
        area_sqft = parse_area_sqft(area_text)

        # Price per sqft
        price_per_sqft = None
        ppsf_text = safe_text(card, "[class*='per-sqft']") or safe_text(card, "[class*='perSqFt']")
        if ppsf_text:
        ...
        # BHK — NoBroker shows "2 BHK Apartment" or similar
        bhk_text = (
            safe_text(card, "[class*='bhk']")
        ...
        # Bathrooms
        bath_text = safe_text(card, "[class*='bath']") or safe_text(card, "[class*='Bathroom']")
        bathrooms = extract_bathrooms(bath_text)

        # Property type
        ptype_text = (
            safe_text(card, "[class*='propertyType']")
            or safe_text(card, "[class*='property-type']")
            or (project_name or "")
        )
        ...
        furnishing_text = (
            ...
            or safe_text(card, "[class*='Furnish']")
        )
        furnishing = classify_furnishing(furnishing_text)

        # Floor info
        floor_text = safe_text(card, "[class*='floor']") or safe_text(card, "[class*='Floor']")
        ...
        status_text = (
            ...
            or safe_text(card, "[class*='avail']")
        )
        construction_status = classify_construction(status_text)

        # Age
        age_text = safe_text(card, "[class*='age']") or safe_text(card, "[class*='Age']")
...
def scrape_locality(driver: webdriver.Chrome, variant: str, standard_locality: str) -> List[Dict]:
    """Scrape all pages for a single locality variant on NoBroker."""
    rows: List[Dict] = []

    for page in range(1, MAX_PAGES + 1):
        # Try primary URL first, then alt URL on first page only
        ...
        scroll_to_load(driver, scrolls=6)
        ...
        try:
            WebDriverWait(driver, 15).until(
                EC.presence_of_element_located((
                    By.CSS_SELECTOR,
                    "[class*='card-container'], [class*='nb-card'], [class*='CardContainer'], article"
                ))
            )
        except Exception:
            # Try alt URL on page 1 only
            if page == 1:
        ...
        # Collect cards — NoBroker uses various class naming conventions
        cards = driver.find_elements(By.CSS_SELECTOR, "[class*='card-container']")
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, "[class*='nb-card']")
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, "[class*='CardContainer']")
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, "[class*='property-card']")
        if not cards:
...
def main() -> None:
    print(f"=== NoBroker Hyderabad Scraper ===")
