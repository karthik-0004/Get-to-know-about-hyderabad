"""
scrape_makaan.py
Selenium scraper for makaan.com Hyderabad real-estate listings.

Usage:
    python scrape_makaan.py
...
OUTPUT_FILE = "makaan_hyderabad.csv"
SOURCE_NAME = "Makaan"
MAX_PAGES = 10
...
# ──────────────────────────────────────────────
# Makaan.com-specific scraping
# ──────────────────────────────────────────────

def build_url(variant: str, page: int) -> str:
    """Build Makaan.com search URL for a locality in Hyderabad."""
    slug = variant.lower().replace(" ", "-").replace(".", "")
    if page == 1:
        return f"https://www.makaan.com/hyderabad-residential-property/buy-property-in-{slug}-hyderabad?listingType=buy"
    return f"https://www.makaan.com/hyderabad-residential-property/buy-property-in-{slug}-hyderabad?listingType=buy&page={page}"


def build_url_alt(variant: str, page: int) -> str:
    """Alternative Makaan.com URL pattern."""
    slug = variant.lower().replace(" ", "-").replace(".", "")
    if page == 1:
        return f"https://www.makaan.com/{slug}-hyderabad-residential-property/buy-property-in-{slug}-hyderabad"
    return f"https://www.makaan.com/{slug}-hyderabad-residential-property/buy-property-in-{slug}-hyderabad?page={page}"
...
def extract_listing(card, standard_locality: str) -> Optional[Dict]:
    """Extract one listing from a Makaan.com property card."""
    try:
        # Project name — Makaan uses seller/project name
        project_name = (
            safe_text(card, ".seller-info .seller-name")
            or safe_text(card, "[class*='projectName']")
            or safe_text(card, "a[class*='listing-title']")
            or safe_text(card, "h2")
            or safe_text(card, ".listing-title-text, .listing-card__title")
        )

        # Price
        price_text = (
            safe_text(card, "[class*='price'], .price")
            or safe_text(card, "[data-type='price']")
            or safe_text(card, ".listing-card__price")
        )
        price_inr = parse_price_inr(price_text)

        # Area
        area_text = (
            safe_text(card, "[class*='size'], .size")
            or safe_text(card, "[class*='area']")
            or safe_text(card, ".listing-card__area")
        )
        ...
        price_per_sqft = None
        ppsf_text = (
            safe_text(card, "[class*='perSqft']")
            or safe_text(card, "[class*='price-per-sqft']")
            or safe_text(card, ".rate-per-sqft")
        )
        if ppsf_text:
        ...
        # BHK — Makaan often shows "3 BHK Apartment in ..."
        bhk_text = (
            safe_text(card, "[class*='bed'], .bed")
            or safe_text(card, "[class*='config']")
            or safe_text(card, ".listing-card__config")
            or (project_name or "")
        )
        ...
        # Bathrooms
        bath_text = (
            safe_text(card, "[class*='bath'], .bath")
            or safe_text(card, "[class*='bathroom']")
        )
        ...
        ptype_text = (
            ...
            or safe_text(card, "[class*='listing-card__type']")
            or safe_text(card, ".listing-type")
        )
        property_type = classify_property_type(ptype_text)
        # Try extracting from title if not found
        if not property_type and project_name:
            property_type = classify_property_type(project_name)

        # Furnishing
        furnishing_text = (
            safe_text(card, "[class*='furnish']")
            or safe_text(card, ".listing-card__furnish")
        )
        furnishing = classify_furnishing(furnishing_text)

        # Floor info
        floor_text = (
            safe_text(card, "[class*='floor']")
            or safe_text(card, ".listing-card__floor")
        )
        floor_number, total_floors, floor_info = extract_floor_info(floor_text)

        # Construction status
        status_text = (
            safe_text(card, "[class*='status']")
            or safe_text(card, "[class*='possession']")
            or safe_text(card, ".listing-card__status")
        )
        construction_status = classify_construction(status_text)

        # Age
        age_text = (
            safe_text(card, "[class*='age']")
            or safe_text(card, ".listing-card__age")
        )
...
def scrape_locality(driver: webdriver.Chrome, variant: str, standard_locality: str) -> List[Dict]:
    """Scrape all pages for a single locality variant on Makaan.com."""
    rows: List[Dict] = []
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        try:
            WebDriverWait(driver, 15).until(
                EC.presence_of_element_located((
                    By.CSS_SELECTOR,
                    ".listing-card, [class*='listing-card'], [class*='srpListing'], .search-result-card"
                ))
            )
        except Exception:
            # Try alt URL on first page
            if page == 1:
                alt_url = build_url_alt(variant, page)
                print(f"    Retrying with alt URL: {alt_url[:100]}...")
                try:
                    driver.get(alt_url)
                    random_delay(2.0, 4.0)
                    scroll_to_load(driver, scrolls=5)
                ...
                print(f"    Page {page}: no listings — stopping pagination")
                break
        ...
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, "[class*='srpListing']")
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, ".search-result-card")
        if not cards:
            cards = driver.find_elements(By.CSS_SELECTOR, "[id^='listing-']")
        ...
        try:
            next_btn = driver.find_element(By.CSS_SELECTOR, "a.next, a[class*='next'], .pagination a:last-child")
...
def main() -> None:
    print(f"=== Makaan.com Hyderabad Scraper ===")
