"""
Housing.com - Hyderabad PLOT Scraper
======================================
1. Reads all locality rows from price-trends table (all 17 pages)
2. For each locality opens listing page e.g. /in/buy/hyderabad/bandlaguda
3. Clicks "Property Type" dropdown → clicks "Plot" link
   e.g. <a href="/in/buy/hyderabad/plot-bandlaguda">Plot</a>
4. Scrapes ALL plot listings with full pagination
5. Saves to plot.xlsx after every locality

Columns: Locality | Avg Price/Sqft | Price Min | Price Max | Sqft | Price | Amenities
...
TOTAL_PAGES = 17
OUTPUT_FILE = "plot.xlsx"
HEADLESS    = False

# ── EXCEL ─────────────────────────────────────────────────────────────────────
# Note: Plots don't have BHK — removed that column
COLUMNS    = ["Locality", "Avg Price/Sqft", "Price Min", "Price Max",
              "Plot Area (Sqft)", "Price", "Amenities"]
COL_WIDTHS = [25, 18, 16, 16, 18, 15, 55]
HDR_FILL   = PatternFill("solid", fgColor="1B5E20")   # dark green for plots
...
def save_locality(wb, ws, locality, avg_price, p_min, p_max, props):
    if not props:
        props = [{"sqft": "", "price": "", "amenities": ""}]
    for prop in props:
        r = ws.max_row + 1
        vals = [locality, avg_price, p_min, p_max,
                prop["sqft"], prop["price"], prop["amenities"]]
...
# ── GET PLOT URL FOR A LOCALITY ───────────────────────────────────────────────

def get_plot_url(page, locality_url):
    """
    Open locality listing page → click Property Type dropdown
    → find Plot <a> link → return full URL.
    """
    print(f"      🔍 Finding Plot URL: {locality_url[:70]}")
    ...
    # Find "Plot" <a> href inside dropdown
    plot_href = page.evaluate("""
        () => {
            // Primary: div.option elements
            const options = document.querySelectorAll('div.option, div[class*="option"]');
            for (const opt of options) {
                if (opt.innerText.trim() === 'Plot') {
                    const link = opt.querySelector('a');
                    if (link) return link.getAttribute('href');
                }
            }
            // Fallback: any <a> with exact text "Plot"
            const links = document.querySelectorAll('a');
            for (const a of links) {
                if (a.innerText.trim() === 'Plot') {
    ...
    if plot_href:
        full_url = ("https://housing.com" + plot_href
                    if plot_href.startswith("/") else plot_href)
        print(f"      ✔ Plot URL: {full_url}")
        return full_url
    else:
        print(f"      ⚠ 'Plot' link not found in dropdown")
...
def extract_fields(txt):
    """Plots don't have BHK — extract sqft, price, amenities."""
    ...
    # Sqft — plot area
    sqft_m = re.search(r"([\d,]+)\s*sq\.?\s*(?:ft|yd|meter|mt)", txt, re.I)
    sqft   = sqft_m.group(0).strip() if sqft_m else ""

    # Also try yards
    if not sqft:
        yard_m = re.search(r"([\d,]+)\s*(?:sq\.?\s*yd|yard)", txt, re.I)
        sqft   = yard_m.group(0).strip() if yard_m else ""

    # Amenities
    amenity_keywords = [
        "Corner Plot", "East Facing", "West Facing", "North Facing", "South Facing",
        "DTCP Approved", "HMDA Approved", "RERA", "Gated Community", "Gated Layout",
        "Water Supply", "Electricity", "Road Facing", "Park Facing",
        "Swimming Pool", "Garden", "Park", "Security", "CCTV",
        "Vastu", "Clear Title", "Compound Wall", "Paved Road",
        "24Hr Water", "Underground Drainage",
    ]
    found     = [a for a in amenity_keywords if a.lower() in txt.lower()]
    amenities = ", ".join(dict.fromkeys(found))

    if not price and not sqft:
        return None
    return {"sqft": sqft, "price": price, "amenities": amenities}
...
def scrape_all_properties(page, plot_url, locality):
    all_props  = []
    page_num   = 1
    total      = None
    last_count = -1

    base_url = re.sub(r"[?&]page=\d+", "", plot_url)
    ...
    while True:
                ...
                for sel in ["div[class*='property']", "div[class*='result']",
                    ...
                    if count_el:
                        ...
                        if m:
                            ...
                            print(f"      ℹ Total Plot listings: {total}")
                            break
            except Exception:
                pass
        ...
        if total and len(all_props) >= total:
            print(f"      ✅ Collected all {total} plots")
            break
    ...
    print(f"      ✅ '{locality}' complete — {len(all_props)} plots")
    return all_props
    ...
    with sync_playwright() as p:
        ...
        for pg in range(1, TOTAL_PAGES + 1):
            ...
            for ld in locality_data:
                ...
                props = []
                if ld["prop_url"]:
                    plot_url = get_plot_url(page, ld["prop_url"])
                    if plot_url:
                        props = scrape_all_properties(page, plot_url, locality)
                    else:
                        print(f"   ⚠ No Plot URL found — locality may not have plots")
